...
def main(model, english,verbose, energy, pause,dynamic_energy, save_entire_clip, session_name, session_timestamp):
    ...
    try:
        ...
        return
    # Terminate if session_name is not provided
    if session_name == "":
        print("Session name and timestamp are required")
    os.makedirs(os.path.abspath('/Users/ziyeliu/Code/play_ground/database/'), exist_ok=True)
    os.makedirs(os.path.abspath('/Users/ziyeliu/Code/play_ground/audio_recording/'), exist_ok=True)
    os.makedirs(os.path.abspath('/Users/ziyeliu/Code/play_ground/audio_recording_temp/'), exist_ok=True)
...
def record_audio(audio_queue, energy, pause, dynamic_energy):
    ...
    with sr.Microphone(sample_rate=16000) as source:
        ...
        while True:
            ...
            wav_file = wave.open(os.path.abspath('/Users/ziyeliu/Code/play_ground/audio_recording_temp/') + '/audio_' + str(i) + '.wav', 'wb')
...
def transcribe_forever(audio_queue, audio_model, english, verbose, session_name, session_timestamp):
    ...
    while True:
        try:
            ...
            # Log exception to transcribe_session database
            c.execute(f"""INSERT INTO transcribe_session VALUES ("{current_timestamp}", "{session_timestamp}", "{e}")""")
            conn.commit()
