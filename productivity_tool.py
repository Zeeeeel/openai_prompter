...
from dir_config import db, audio_temp_dir, audio_dir, app_data_dir, error_log_dir
import logging

# Set up the logging configuration
logging.basicConfig(filename=error_log_dir + '/productivity_tool_error.log', level=logging.ERROR)
print(f"ERROR LOG FOR productivity_tool.py IS STORED AT {error_log_dir}")

# import transcribe
# For automation use the following
# cd /Users/ziyeliu/Code/OAIToolKit/ && usr/local/bin/python3 ~/Code/OAIToolKit/productivity_tool.py 
        ...
        if self.transcribe_started:
            ...
            for filename in os.listdir(audio_temp_dir):
                if filename.endswith('.wav'):
                    # Load the audio file
                    audio = AudioSegment.from_file(os.path.join(audio_temp_dir, filename))
                    # Append the audio file to the consolidated audio
                    consolidated_audio += audio

            # Export the consolidated audio to a new file
            consolidated_audio.export(audio_dir + "consolidated_audio.wav", format="wav")
            # Delete delete temp folder
            os.system("rm -r {audio_temp_dir}")
...
class TranscribeThread(threading.Thread):
    ...
    def run(self):
        # If the current owner is not the current user, change the owner
        
        self.transcribe_process = subprocess.Popen([sys.executable, 'transcribe.py', '--model', 'small', '--pause', '0.6', '--session_name', self.session_name, '--session_timestamp', self.timestamp], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        atexit.register(self.stop)

        # Get the output from the other program
        last_checked_timestamp = 0
        while self.stopped == False and self.transcribe_process.poll() is None:
            try:
            ...
            sleep(0.1)
        # Read the remaining output after the subprocess has finished
        (stdout, stderr) = self.transcribe_process.communicate()
        if stdout:
